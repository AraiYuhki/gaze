...
- [ ] 文字入力/削除
- [ ] 複数行対応
- [ ] ステージされたファイル一覧を表示（コミット対象の確認用）
- [ ] `j`/`k` でファイル一覧のカーソル移動
- [ ] `d` キーで選択ファイルの staged diff を表示（`git diff --staged <file>`）
...
### 5-2: コミット実行
- [ ] `c` キーでコミットモード開始（Status View から）
- [ ] ステージされたファイルがない場合はエラー表示
- [ ] `Enter` でコミット実行（`git commit -m "<message>"`）
- [ ] `Esc` でキャンセル
- [ ] コミット成功後に status を再取得
...
### 5-3: Amend 機能
- [ ] `C` キー（Shift+c）で amend モード開始
- [ ] 直前のコミットメッセージを取得して表示（`git log -1 --pretty=%B`）
- [ ] `git commit --amend -m "<message>"` を実行
- [ ] 確認ダイアログを表示（amend は履歴を書き換えるため）
...
```
- [ ] **Phase 5 完了**
- [ ] **Phase 5 レビュー完了** - `docs/review/phase-5.md` 作成済み
