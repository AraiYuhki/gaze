...
### 1-1: ドメインモデル
- [ ] `src/domain/mod.rs` を作成
- [ ] `src/domain/status.rs` に `StatusKind`, `FileStatus` を実装
...
### 1-3: AppState
- [ ] `src/app.rs` に `AppState` を実装
- [ ] 現在の View（Status/Tree/Log）を保持
- [ ] Status View の選択状態を保持
- [ ] `status_cache: Vec<FileStatus>` を保持
- [ ] `refresh_status()` メソッドを実装
...
### 1-4: Status View UI
- [ ] `src/ui/mod.rs` を作成
- [ ] `src/ui/status_view.rs` を実装
- [ ] ファイル一覧表示
- [ ] ステータスの色分け（M=黄, A=緑, D=赤, ?=灰）
- [ ] カーソル表示
...
- [ ] s キーでステージ/アンステージ切替
- [ ] `git add <file>` 実行
- [ ] `git restore --staged <file>` 実行
- [ ] 操作完了後に status を1回だけ再取得
...
**禁止事項の確認**:
- [ ] 描画ループ内で git status を実行していないこと
- [ ] キー入力毎に git status を実行していないこと

### 1-7: 差分表示
- [ ] d キーで差分表示
- [ ] `git diff <file>` の出力を取得
- [ ] 外部ページャで表示
- [ ] ページャ失敗時はエラー表示（アプリは継続）
...
```
- [ ] **Phase 1 完了** <!-- 完了日時: -->
- [ ] **Phase 1 レビュー完了** - `docs/review/phase-1.md` 作成済み
